...
# ============================================================
# Within-AUG start-competence contrasts
# ============================================================

WITHIN_AUG_SUB_COLS <- c(
  "substitution_A_te_change",
  "substitution_C_te_change",
  "substitution_G_te_change",
  "substitution_T_te_change"
)
missing_within_aug_cols <- setdiff(WITHIN_AUG_SUB_COLS, names(pos_sub_aug.df))
if (length(missing_within_aug_cols) > 0) {
  stop(
    "Position summaries are missing per-alt-base ΔTE columns: ",
    paste(missing_within_aug_cols, collapse = ", "),
    ". Rerun the mutagenesis summarization with the updated ",
    "summarize_shard.py before making within-AUG contrast plots."
  )
}

within_aug_class_levels <- c(
  "strong NC\nincl. ACG",
  "weak NC\nincl. AUU",
  "non-start\nAAG / AGG",
  "deletion\nfull loss"
...
within_aug_substitution_long.df <- pos_sub_aug.df %>%
  filter(
    target_codon %in% c("ATG", "AUG"),
    orf_position_in_start_codon %in% 1:3
  ) %>%
  mutate(across(all_of(WITHIN_AUG_SUB_COLS), ~suppressWarnings(as.numeric(.x)))) %>%
  pivot_longer(
    cols = all_of(WITHIN_AUG_SUB_COLS),
    names_to = "alt_base",
    values_to = "delta_te"
  ) %>%
  mutate(
    alt_base = recode(
      ...
      substitution_C_te_change = "C",
      substitution_G_te_change = "G",
      substitution_T_te_change = "T"
    ),
    mutated_codon = case_when(
      orf_position_in_start_codon == 1 ~ paste0(alt_base, "TG"),
      orf_position_in_start_codon == 2 ~ paste0("A", alt_base, "G"),
      orf_position_in_start_codon == 3 ~ paste0("AT", alt_base),
      TRUE ~ NA_character_
    ),
    mutated_codon_rna = str_replace_all(mutated_codon, "T", "U"),
    codon_class_model = case_when(
      mutated_codon %in% c("CTG", "GTG", "TTG", "ACG") ~ "strong_nc",
      mutated_codon %in% c("ATA", "ATC", "ATT")        ~ "weak_nc",
      mutated_codon %in% c("AAG", "AGG")               ~ "non_start",
      TRUE ~ NA_character_
    ),
    codon_class = recode(
      codon_class_model,
  ...
  filter(!is.na(codon_class_model), is.finite(delta_te))

within_aug_substitution_orf.df <- within_aug_substitution_long.df %>%
  group_by(
    orf_id, gene_id, facet_group, detected_group, orf_definition_sub,
    codon_class_model, codon_class
  ) %>%
  summarise(
    delta_te = mean(delta_te, na.rm = TRUE),
    n_variants = n(),
    .groups = "drop"
  )

within_aug_deletion_orf.df <- ism_sub.df %>%
  filter(!is.na(facet_group), !is.na(detected_group)) %>%
  transmute(
    orf_id,
    gene_id,
    facet_group,
    detected_group,
    orf_definition_sub,
    codon_class_model = "deletion",
    codon_class = "deletion\nfull loss",
    delta_te = suppressWarnings(as.numeric(orf_deletion_te_change_mean_3nt)),
    n_variants = 3L
  ) %>%
  filter(is.finite(delta_te))

within_aug_staircase_orf.df <- bind_rows(
  within_aug_substitution_orf.df,
  within_aug_deletion_orf.df
) %>%
  mutate(
    codon_class = factor(codon_class, levels = within_aug_class_levels),
    detected_group = factor(detected_group, levels = c("Detected", "Not detected"))
  )

within_aug_staircase_summary.df <- within_aug_staircase_orf.df %>%
  group_by(facet_group, detected_group, codon_class) %>%
  summarise(
    mean_delta = mean(delta_te, na.rm = TRUE),
    se_delta = sd(delta_te, na.rm = TRUE) / sqrt(n()),
    n_orfs = n(),
    .groups = "drop"
  )

within_aug_pos2_contrast.df <- within_aug_substitution_long.df %>%
  filter(
    orf_position_in_start_codon == 2,
    mutated_codon %in% c("ACG", "AAG", "AGG")
  ) %>%
  group_by(orf_id, gene_id, facet_group, detected_group, orf_definition_sub) %>%
  summarise(
    acg_delta = mean(delta_te[mutated_codon == "ACG"], na.rm = TRUE),
    aag_agg_delta = mean(delta_te[mutated_codon %in% c("AAG", "AGG")],
                         na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(aag_agg_minus_acg = aag_agg_delta - acg_delta) %>%
  filter(is.finite(aag_agg_minus_acg)) %>%
  mutate(detected_group = factor(detected_group,
                                 levels = c("Detected", "Not detected")))

empty_within_aug_contrast_row <- function(status, message = NA_character_) {
  tibble(
    status = status,
    message = message,
    n_detected = NA_integer_,
    n_undetected = NA_integer_,
    median_detected = NA_real_,
    median_undetected = NA_real_,
    median_difference = NA_real_,
    wilcox_location_shift = NA_real_,
    wilcox_ci_low = NA_real_,
    wilcox_ci_high = NA_real_,
    wilcox_p = NA_real_
  )
}

summarise_within_aug_pos2_contrast <- function(df) {
  det_values <- df$aag_agg_minus_acg[df$detected_group == "Detected"]
  undet_values <- df$aag_agg_minus_acg[df$detected_group == "Not detected"]
  if (length(det_values) < 3 || length(undet_values) < 3) {
    return(empty_within_aug_contrast_row(
      "skipped",
      "Need at least 3 detected and 3 undetected ORFs."
    ) %>%
      mutate(
  ...
  wt <- tryCatch(
    wilcox.test(
      aag_agg_minus_acg ~ detected_group,
      data = df,
      exact = FALSE,
      conf.int = TRUE
    ),
  ...
  if (inherits(wt, "error")) {
    return(empty_within_aug_contrast_row("error", conditionMessage(wt)) %>%
      mutate(
        n_detected = length(det_values),
        n_undetected = length(undet_values),
        median_detected = median(det_values, na.rm = TRUE),
        median_undetected = median(undet_values, na.rm = TRUE),
        median_difference = median(det_values, na.rm = TRUE) -
          median(undet_values, na.rm = TRUE)
      ))
  }
  ...
  tibble(
    ...
    median_difference = median(det_values, na.rm = TRUE) -
      median(undet_values, na.rm = TRUE),
    wilcox_location_shift = unname(wt$estimate),
    wilcox_ci_low = wt$conf.int[1],
    wilcox_ci_high = wt$conf.int[2],
    wilcox_p = wt$p.value
  )
}

within_aug_pos2_contrast_stats.df <- within_aug_pos2_contrast.df %>%
  group_by(facet_group) %>%
  group_modify(~summarise_within_aug_pos2_contrast(.x)) %>%
  ungroup()

within_aug_pos2_contrast_stats.df

empty_within_aug_interaction_row <- function(status, message = NA_character_) {
  tibble(
    status = status,
    message = message,
    term = NA_character_,
...
fit_within_aug_interaction <- function(df) {
  model_df <- df %>%
    filter(codon_class_model != "deletion") %>%
    mutate(
      codon_class_model = factor(codon_class_model,
                                 levels = c("strong_nc", "weak_nc", "non_start")),
      detected_model = factor(detected_group,
                              levels = c("Not detected", "Detected")),
      orf_re = factor(orf_id)
    )

  if (n_distinct(model_df$detected_model) < 2 ||
      n_distinct(model_df$codon_class_model) < 3 ||
      n_distinct(model_df$orf_re) < 3) {
    return(empty_within_aug_interaction_row(
      "skipped",
      "Need both detection groups, all substitution classes, and at least 3 ORFs."
    ) %>%
      mutate(n = nrow(model_df), n_orfs = n_distinct(model_df$orf_re)))
  }

  fit <- tryCatch(
    mgcv::gam(
      delta_te ~ codon_class_model * detected_model + s(orf_re, bs = "re"),
      data = model_df,
      method = "REML"
    ),
    error = function(err) err
  ...
  if (inherits(fit, "error")) {
    return(empty_within_aug_interaction_row("error", conditionMessage(fit)) %>%
      mutate(n = nrow(model_df), n_orfs = n_distinct(model_df$orf_re)))
  }

  term_labels <- c(
    detected_modelDetected = "detected gap at strong NC",
    `codon_class_modelweak_nc:detected_modelDetected` =
      "detected slope: weak NC vs strong NC",
    `codon_class_modelnon_start:detected_modelDetected` =
      "detected slope: non-start vs strong NC"
  )
  param_table <- as.data.frame(summary(fit)$p.table)
  ...
  param_table %>%
    ...
    transmute(
      ...
      message = NA_character_,
      term,
      contrast = unname(term_labels[term]),
      estimate = Estimate,
      se = `Std. Error`,
      ...
      p = `Pr(>|t|)`,
      n = nrow(model_df),
      n_orfs = n_distinct(model_df$orf_re)
    )
}

within_aug_interaction_results.df <- within_aug_staircase_orf.df %>%
  group_by(facet_group) %>%
  group_modify(~fit_within_aug_interaction(.x)) %>%
  ungroup()

within_aug_interaction_results.df

within_aug_interaction_coef.gg <- within_aug_interaction_results.df %>%
  filter(status == "ok", term != "detected_modelDetected") %>%
  mutate(
    contrast = factor(
      contrast,
      levels = c(
  ...
  ggplot(aes(x = contrast, y = estimate, ymin = ci_low, ymax = ci_high,
             colour = facet_group)) +
  geom_hline(yintercept = 0, colour = "grey55", linetype = "dashed",
             linewidth = 0.35) +
  geom_pointrange(position = position_dodge(width = 0.45), linewidth = 0.45) +
  ...
  labs(
    x = NULL,
    y = "Interaction coefficient (ΔTE)",
    title = "Substitution-only start-competence interaction"
  ) +
  pub_base_theme +
  theme(axis.text.x = element_text(angle = 25, hjust = 1))

make_within_aug_staircase_plot <- function(structural_type, detected_color) {
  plot_df <- within_aug_staircase_summary.df %>%
    filter(facet_group == structural_type)
  ggplot(plot_df, aes(x = codon_class, y = mean_delta,
                      group = detected_group,
                      colour = detected_group,
                      linetype = detected_group)) +
    ...
    geom_errorbar(aes(ymin = mean_delta - 1.96 * se_delta,
                  ...
                  width = 0.08, linewidth = 0.35, alpha = 0.65) +
    geom_line(linewidth = 0.7) +
    geom_point(size = 1.8) +
    scale_colour_manual(
      values = c("Detected" = detected_color, "Not detected" = "#8A8A84"),
      name = NULL
    ) +
    scale_linetype_manual(
      values = c("Detected" = "solid", "Not detected" = "dashed"),
      name = NULL
    ) +
    labs(
      x = "start-codon competence retained → abolished",
      y = "Mean ΔTE",
      title = structural_type
    ) +
    pub_base_theme +
...
make_within_aug_contrast_plot <- function(structural_type, detected_color) {
  plot_df <- within_aug_pos2_contrast.df %>%
    filter(facet_group == structural_type)
  stat_df <- within_aug_pos2_contrast_stats.df %>%
    filter(facet_group == structural_type) %>%
    mutate(label = if_else(
      status == "ok",
      paste0("MW p=", format.pval(wilcox_p, digits = 2, eps = 1e-3)),
      status
    ))

  ggplot(plot_df, aes(x = detected_group, y = aag_agg_minus_acg,
                      fill = detected_group, colour = detected_group)) +
    geom_hline(yintercept = 0, colour = "grey55", linetype = "dashed",
               linewidth = 0.35) +
    geom_boxplot(alpha = 0.7, outlier.shape = NA, width = 0.48,
                 colour = "grey30", linewidth = 0.35) +
    geom_jitter(width = 0.12, alpha = 0.35, size = 0.55, stroke = 0,
                show.legend = FALSE) +
    geom_text(
    ...
    scale_fill_manual(
      values = c("Detected" = detected_color, "Not detected" = "#C7C7C7"),
      name = NULL
    ) +
    scale_colour_manual(
      values = c("Detected" = detected_color, "Not detected" = "#8A8A84"),
...
within_aug_staircase_uorf.gg <- make_within_aug_staircase_plot("uORF", "#1B9E77")
within_aug_staircase_uo_orf.gg <- make_within_aug_staircase_plot("uoORF", "#74669d")
within_aug_staircase.gg <- cowplot::plot_grid(
  within_aug_staircase_uorf.gg,
  within_aug_staircase_uo_orf.gg,
  ncol = 2,
  align = "hv"
)

within_aug_contrast_uorf.gg <- make_within_aug_contrast_plot("uORF", "#1B9E77")
within_aug_contrast_uo_orf.gg <- make_within_aug_contrast_plot("uoORF", "#74669d")
within_aug_contrast.gg <- cowplot::plot_grid(
  within_aug_contrast_uorf.gg,
  within_aug_contrast_uo_orf.gg,
  ncol = 2,
  align = "hv"
)
